      ...
      <div key={exam.id} className="my-1 sm:my-4 mx-1 sm:mx-8">
        <div className="flex flex-col sm:flex-row justify-between items-center">
          <div className="flex flex-row mb-3">
            <h1 className="text-xl sm:text-2xl font-bold">{`Q${exam.questions[currentPage - 1].No}:`}</h1>
            <p className="max-w-xl text-xl sm:text-lg leading-0 sm:leading-8 text-gray-600 mx-3">
              {exam.questions[currentPage - 1].question}
            </p>
          </div>
  ...
  <Tooltip content="Bolt">
    <Button className="text-white w-8 md:w-full bg-indigo-600 border border-gray-200 p-2 px-8" style={{ opacity: 0.7 }}>
      <FaBolt />
        ...
        <div className="flex flex-col mt-5">
          ...
          {exam.questions[currentPage - 1].options.map((option, index) => (
            <div key={index} className="flex flex-row space-x-2 my-5">
              <Button
                className={`bg-${selectedAnswer === option && selectedAnswer === exam.questions[currentPage - 1].correctAnswer ? 'green-500' : 'indigo-600'} text-white rounded-md`}
                onClick={() => handleAnswerClick(option)}
              >
                <h2 className='text-xl font-medium'>{exam.questions[currentPage - 1].option[index]}</h2>
              </Button>
    ...
    <div className="flex overflow-x-auto sm:justify-center mb-3">
      <Pagination
        ...
        totalPages={Math.ceil(quizQuestions.exams[0].questions.length / questionsPerPage)}
